...
void setup() {
  Wire.begin();                        // Initialize I2C-communication as master
                                       //    SDA on pin ADC04
                                       //    SCL on pin ADC05
  SetFrequency (146000000);             // Set TX-Frequency [10,14 MHz]
...
void TX_ON () {                        // Enables output on CLK0 and disables Park Mode on CLK1
  ...
  Si5351a_Write_Reg (16, 111);          // Enable output CLK0, set crystal as source and Integer Mode on PLLA
}
...
void SetFrequency (unsigned long frequency) { // Frequency in Hz; must be within [7810 Hz to 200 Mhz]
  ...
  Si5351a_Write_Reg (162, 0x74);
  Si5351a_Write_Reg (163, 0xEE);
  Si5351a_Write_Reg (164, 0x01);  
...
void SetPower (byte regist, byte power){                         // Sets the output power level
  if (power == 0 || power > 4){power = 0;}          // valid power values are 0 (25%), 1 (50%), 2 (75%) or 3 (100%)
  switch (power){
    case 1:
      Si5351a_Write_Reg (regist, 76);                   // CLK0 drive strength = 2mA; power level ~ -8dB
      break;
    case 2:
      Si5351a_Write_Reg (regist, 77);                   // CLK0 drive strength = 4mA; power level ~ -3dB
      break;
    case 3:
      Si5351a_Write_Reg (regist, 78);                   // CLK0 drive strength = 6mA; power level ~ -1dB
      break;
    case 4:
      Si5351a_Write_Reg (regist, 79);                   // CLK0 drive strength = 8mA; power level := 0dB
      break;
  }
}
